...
app.post('/api/measurements', async (req, res) => {
    const { item_id, details, bill_no, customer_name } = req.body; 
    try {
        // QUICK FIX: Force item_id to be a valid integer number (like 1) 
        // so PostgreSQL doesn't throw a type syntax error for text strings
        const numericItemId = 1; 

        const result = await pool.query(
            'INSERT INTO measurements (item_id, details, bill_no, customer_name) VALUES ($1, $2, $3, $4) RETURNING *',
            [numericItemId, JSON.stringify(details), bill_no, customer_name] 
        );
...
// Get all saved measurements from the database
